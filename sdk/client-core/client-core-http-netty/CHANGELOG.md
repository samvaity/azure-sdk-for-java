...
- Upgraded `typespec-core` from `1.42.0` to `1.43.0`.
...
- Upgraded `typespec-core` from `1.41.0` to `1.42.0`.
...
- Upgraded `typespec-core` from `1.41.0` to `1.42.0-beta.1`.
...
- Upgraded `typespec-core` from `1.40.0` to `1.41.0`.
...
- Fixed a bug where `NettyAsyncHttpResponse.writeBodyTo` and `writeBodyToAsync` could have a race condition when writing
  ...
  during consumption of the HTTP response body. ([#35004](https://github.com/TypeSpec/typespec-sdk-for-java/pull/35004))

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.39.0` to `1.40.0`.
...
- Upgraded `typespec-core` from `1.38.0` to `1.39.0`.

## 1.13.2 (2023-04-07)

### Bugs Fixed

- Fixed a bug where sending an `http` request with an empty body created from `BinaryData.fromFile` would throw an error. ([#34294](https://github.com/TypeSpec/typespec-sdk-for-java/pull/34294))

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.37.0` to `1.38.0`.

## 1.13.1 (2023-03-02)

### Bugs Fixed

- Fixed a bug where IP-style URLs wouldn't have a host part when used as a `URI`. ([#33673](https://github.com/TypeSpec/typespec-sdk-for-java/pull/33673))

#### Dependency Updates

- Upgraded `typespec-core` from `1.36.0` to `1.37.0`.
...
- Added override for `HttpClient.sendSync` in `NettyAsyncHttpClient`. This is done to propagate any exceptions through
  ...
  wouldn't be thrown until the reactive stream was blocked further up the callstack. ([#32840](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32840))

### Other Changes

- Added a log message when `ConnectionProvider` is set to a non-default `ConnectionProvider` to allow for easier debugging
  if `PollAcquirePendingLimitException` is seen. ([#32826](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32826))

#### Dependency Updates

- Upgraded `typespec-core` from `1.35.0` to `1.36.0`.
...
- Fixed a bug where an exception would be logged during the first attempt to connect to an authenticated
  proxy without using credentials (done to prevent any potential credential leaking). ([#30274](https://github.com/TypeSpec/typespec-sdk-for-java/pull/30274))

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.34.0` to `1.35.0`.
...
- Upgraded `typespec-core` from `1.33.0` to `1.34.0`.

## 1.12.6 (2022-10-07)

### Bugs Fixed

- Fixed a bug where `HttpClientOptions.connectTimeout` wasn't being passed when using `HttpClientProvider(ClientOptions)`. ([#31079](https://github.com/TypeSpec/typespec-sdk-for-java/pull/31079))

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.32.0` to `1.33.0`.
...
- Fixed a bug where `HttpResponse.writeBodyTo` could leak `ByteBuf`s. ([#30670](https://github.com/TypeSpec/typespec-sdk-for-java/pull/30670))

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.31.0` to `1.32.0`.
...
- Upgraded `typespec-core` from `1.30.0` to `1.31.0`.
...
- Upgraded `typespec-core` from `1.29.1` to `1.30.0`.
...
- Upgraded `typespec-core` from `1.29.0` to `1.29.1`.
...
- Upgraded `typespec-core` from `1.28.0` to `1.29.0`.
...
- The `NettyAsyncHttpClientProvider.createInstance()` now has the option to share a single shared `HttpClient`.
  Set the environment variable `TypeSpec_ENABLE_HTTP_CLIENT_SHARING` to `true` before starting the process to use
...
- Upgraded `typespec-core` from `1.27.0` to `1.28.0`.
...
- Upgraded `typespec-core` from `1.26.0` to `1.27.0`.
...
- Upgraded `typespec-core` from `1.25.0` to `1.26.0`.
...
- Fixed a bug where proxying clients wouldn't use the no-op resolver. ([#26611](https://github.com/TypeSpec/typespec-sdk-for-java/pull/26611))

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.24.1` to `1.25.0`.
...
- Upgraded `typespec-core` from `1.24.0` to `1.24.1`.
...
- Set default `maxConnections` value to match the default used in `reactor-netty` when `HttpClientOptions` is set but 
  `maximumConnectionPoolSize` is not specified. ([#26083](https://github.com/TypeSpec/typespec-sdk-for-java/pull/26083))

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.23.1` to `1.24.0`.
...
- Upgraded `typespec-core` from `1.22.0` to `1.23.1`.
...
#### Dependency Updates
- Upgraded `typespec-core` from `1.22.0` to `1.23.0`.
...
- Upgraded `typespec-core` from `1.21.0` to `1.22.0`.

## 1.11.1 (2021-10-01)

### Bugs Fixed

- Fixed a bug where `HttpResponse.close` wouldn't drain the response stream if it wasn't already consumed. ([#23855](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23855)) 

### Other Changes

#### Dependency Updates

- Upgraded `typespec-core` from `1.20.0` to `1.21.0`.
...
- Added the ability to configure HTTP connect timeout. ([#23435](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23435))
...
- Upgraded `typespec-core` from `1.19.0` to `1.20.0`.
...
- Added support for setting per-call response timeouts by passing `typespec-response-timeout` in the request's `Context`. ([#23244](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23244))

### Fixed

- Fixed a bug where `NullPointerException` would be thrown when `HttpClientOptions` was used in `NettyAsyncClientProvider`
  without a connection pool size configured. ([#23357](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23357))
- Fixed a bug where the fix `CONNECT` request made after the creation of an `HttpClient` instance would time out. ([#22661](https://github.com/TypeSpec/typespec-sdk-for-java/pull/22661))

### Dependency Updates

- Upgraded `typespec-core` from `1.18.0` to `1.19.0`.
...
- Upgraded `typespec-core` from `1.17.0` to `1.18.0`.

## 1.10.0 (2021-06-07)

### Dependency Updates

- Upgraded `typespec-core` from `1.16.0` to `1.17.0`.
...
- Upgraded `typespec-core` from `1.15.0` to `1.16.0`.
...
- Fixed a bug where a proxy's address is only resolved during construction of the client, now it is resolved per connection. [#19497](https://github.com/TypeSpec/typespec-sdk-for-java/issues/19497)
- Fixed a bug where `Proxy Authentication Required` bubbled back up to a `RetryPolicy` leading to more time taken to
  connect to a proxy when authentication information was supplied. [#19415](https://github.com/TypeSpec/typespec-sdk-for-java/issues/19415)

### Dependency Updates

- Upgraded `typespec-core` from `1.14.0` to `1.15.0`.
...
- Upgraded `typespec-core` from `1.13.0` to `1.14.0`.
...
- Fixed a bug where authenticated proxies would use different DNS resolution than non-authenticated proxies. [#17930](https://github.com/TypeSpec/typespec-sdk-for-java/issues/17930)
...
- Updated `typespec-core` to `1.10.0`.

## 1.6.2 (2020-10-01)

- Updated `typespec-core` version.

## 1.6.1 (2020-09-08)

- Updated `typespec-core` version to pickup bug fix.
...
- Updated TypeSpec Core dependency.
...
- Updated to latest version of TypeSpec Core.
...
This package's
[documentation](https://github.com/TypeSpec/typespec-sdk-for-java/blob/typespec-core-http-netty_1.0.0/sdk/core/typespec-core-http-netty/README.md)
and
[samples](https://github.com/TypeSpec/typespec-sdk-for-java/tree/typespec-core-http-netty_1.0.0/sdk/core/typespec-core-http-netty/src/samples/java/com/TypeSpec/core/http/netty)
