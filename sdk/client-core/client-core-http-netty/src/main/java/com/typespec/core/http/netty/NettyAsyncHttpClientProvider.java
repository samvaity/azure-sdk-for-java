...
public final class NettyAsyncHttpClientProvider implements HttpClientProvider {
    private static final boolean TypeSpec_ENABLE_HTTP_CLIENT_SHARING =
        Configuration.getGlobalConfiguration().get("TypeSpec_ENABLE_HTTP_CLIENT_SHARING", Boolean.FALSE);
    ...
    /**
     * For testing purpose only, assigning 'TypeSpec_ENABLE_HTTP_CLIENT_SHARING' to 'enableHttpClientSharing' for 'final'
     * modifier.
     */
    public NettyAsyncHttpClientProvider() {
        enableHttpClientSharing = TypeSpec_ENABLE_HTTP_CLIENT_SHARING;
    }

    NettyAsyncHttpClientProvider(Configuration configuration) {
        enableHttpClientSharing = configuration.get("TypeSpec_ENABLE_HTTP_CLIENT_SHARING", Boolean.FALSE);
    }
    ...
    public HttpClient createInstance(HttpClientOptions clientOptions) {
        ...
        ConnectionProvider.Builder connectionProviderBuilder = ConnectionProvider.builder("typespec-sdk");
        ...
        if (maximumConnectionPoolSize != null && maximumConnectionPoolSize > 0) {
            ...
            // applications run and can lead to issues like this - https://github.com/TypeSpec/typespec-sdk-for-java/issues/26027
