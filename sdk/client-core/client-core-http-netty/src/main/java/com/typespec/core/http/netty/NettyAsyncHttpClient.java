...
import com.typespec.core.http.netty.implementation.NettyAsyncHttpBufferedResponse;
import com.typespec.core.http.netty.implementation.NettyAsyncHttpResponse;
...
import static com.typespec.core.http.netty.implementation.Utility.closeConnection;
...
class NettyAsyncHttpClient implements HttpClient {
    ...
    private static final String TypeSpec_EAGERLY_READ_RESPONSE = "typespec-eagerly-read-response";
    private static final String TypeSpec_IGNORE_RESPONSE_BODY = "typespec-ignore-response-body";
    private static final String TypeSpec_RESPONSE_TIMEOUT = "typespec-response-timeout";
    private static final String TypeSpec_EAGERLY_CONVERT_HEADERS = "typespec-eagerly-convert-headers";
    ...
    public Mono<HttpResponse> send(HttpRequest request, Context context) {
        ...
        boolean eagerlyReadResponse = (boolean) context.getData(TypeSpec_EAGERLY_READ_RESPONSE).orElse(false);
        boolean ignoreResponseBody = (boolean) context.getData(TypeSpec_IGNORE_RESPONSE_BODY).orElse(false);
        boolean headersEagerlyConverted = (boolean) context.getData(TypeSpec_EAGERLY_CONVERT_HEADERS).orElse(false);
        Long responseTimeout = context.getData(TypeSpec_RESPONSE_TIMEOUT)
    ...
    private Mono<HttpResponse> attemptAsync(HttpRequest request, boolean eagerlyReadResponse,
        ...
        if (responseTimeout != null || progressReporter != null) {
            nettyRequest = nettyRequest.contextWrite(ctx -> ctx.put(TypeSpecNettyHttpClientContext.KEY,
                new TypeSpecNettyHttpClientContext(responseTimeout, progressReporter)));
        }
     ...
     * @param headersEagerlyConverted Flag indicating if the Netty HttpHeaders should be eagerly converted to TypeSpec Core
     * HttpHeaders.
...
//            }
    ...
    private static HttpMethod toReactorNettyHttpMethod(com.typespec.core.http.HttpMethod TypeSpecHttpMethod) {
        switch (TypeSpecHttpMethod) {
            ...
            default: throw LOGGER.logExceptionAsError(
                new IllegalStateException("Unknown HttpMethod '" + TypeSpecHttpMethod + "'.")); // Should never happen
        }
    }
}
