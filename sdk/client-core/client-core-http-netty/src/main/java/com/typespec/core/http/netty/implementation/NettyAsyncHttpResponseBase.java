...
public abstract class NettyAsyncHttpResponseBase extends HttpResponse {
    ...
    // from Netty HttpHeaders to typespec-core HttpHeaders. Instead, by wrapping it, there is no cost to pay as we map
    // the Netty HttpHeaders API into the typespec-core HttpHeaders API.
    //private NettyToTypeSpecCoreHttpHeadersWrapper headers;
    ...
    NettyAsyncHttpResponseBase(HttpClientResponse reactorNettyResponse, HttpRequest httpRequest,
        ...
        if (headersEagerlyConverted) {
            ...
            this.headers = new NettyToTypeSpecCoreHttpHeadersWrapper(nettyHeaders);
        }
    }
