...
- Fixed a blocking call reported by `BlockHound`. ([#36384](https://github.com/TypeSpec/typespec-sdk-for-java/pull/36384))
...
- Fixed a bug where unknown status was treated as complete status, 
  ...
  ([#35628](https://github.com/TypeSpec/typespec-sdk-for-java/issues/35628))
- Fixed a bug where JacksonDatabind215 throws AccessControlException under SecurityManager
  ([#36187](https://github.com/TypeSpec/typespec-sdk-for-java/issues/36187))
- Prevent FluxUtil from using MappedByteBuffer on Windows to fix a Java runtime behavior bug in Windows
  ([#36168](https://github.com/TypeSpec/typespec-sdk-for-java/pull/36168))
...
- `TypeSpecKeyCredential` now to extends from `KeyCredential`.

### Bugs Fixed

- Fixed a bug where `PagedIterable.mapPage` would result in a `NullPointerException`. ([#35123](https://github.com/TypeSpec/typespec-sdk-for-java/pull/35123))
...
- Added `CoreUtils.randomUuid` to replace usage of `UUID.randomUUID`. In some cases `UUID.randomUUID` used a blocking 
  call whereas `CoreUtils.randomUuid` should never block. ([#34790](https://github.com/TypeSpec/typespec-sdk-for-java/pull/34790))
- Added support for prefixes in `TypeSpecKeyCredentialPolicy`. ([#35010](https://github.com/TypeSpec/typespec-sdk-for-java/pull/35010))
- Added the ability to configure a backoff strategy for `FluxUtil.createRetriableDownloadFlux`. Previous retries 
  wouldn't backoff which could result in requests being sent to a service already at capacity and throttling. ([#35035](https://github.com/TypeSpec/typespec-sdk-for-java/pull/35035))

### Bugs Fixed

- Fixed a bug where a known length wasn't passed to `BinaryData.fromStream` resulting it being handled as a 
  non-replayable `BinaryData`. ([#34851](https://github.com/TypeSpec/typespec-sdk-for-java/pull/34851))
- Changed the design of how `AsynchronousByteChannel`s were written to limit chances of race conditions between the
  ...
  behavior at the cost of lower throughput, which will be investigated in future releases. ([#35004](https://github.com/TypeSpec/typespec-sdk-for-java/pull/35004))

### Other Changes

- Changed how `ResponseError` is deserialized to support cases where the JSON wrapped the `ResponseError` with an 
  `error` property. ([#35052](https://github.com/TypeSpec/typespec-sdk-for-java/pull/35052))
...
- Fixed an edge case when parsing query parameters without a value (`key&key2=value2`). ([#34459](https://github.com/TypeSpec/typespec-sdk-for-java/pull/34459))

### Other Changes

- Changed how handling supporting multiple versions of Jackson happens internally to reduce usage of reflection. ([#34468](https://github.com/TypeSpec/typespec-sdk-for-java/pull/34468))
...
- Added dependency on `typespec-json` to provide stream-style JSON serialization support for `JsonSerializable`
...
- Added support for `JsonSerializable` and `XmlSerializable` in `JacksonAdapter` serialize and deserialize APIs. ([#33685](https://github.com/TypeSpec/typespec-sdk-for-java/pull/33685))
...
- Removed `jackson-dataformat-xml` as a dependency. XML serialization is still supported when `jackson-dataformat-xml`
  ...
  serialization. ([#33401](https://github.com/TypeSpec/typespec-sdk-for-java/pull/33401))

### Bugs Fixed

- Fixed a bug where a method not found error would be thrown when attempting to reflectively access Jackson Databind 2.12+
  APIs. ([#33448](https://github.com/TypeSpec/typespec-sdk-for-java/pull/33448))
...
- Added explicit support for `text`-based serialization, this is done by adding `SerializerEncoding.TEXT`. Previously,
  ...
  wasn't a JSON string (`string` vs `"string"`). ([#32277](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32277))
- Added `BinaryData.fromListByteBuffer(List<ByteBuffer>)` to support additional ways to create `BinaryData`. ([#32932](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32932))
- Added `TracingOptions` to make tracing configurable. ([#32573](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32573))
...
- Fixed a bug where `PollingStrategy.getResult` would guard getting results for `POST`-based on containing a `Location`
  ...
  as the final result. ([#32815](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32815))
- Fixed a bug where `HEAD`-based requests were checking for a body before deserializing. `HEAD` requests shouldn't have
  a body or if they do it should be ignored. ([#32833](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32833))
...
- Fixed a bug where cancellation would result in an application stall by using `doFinally` instead of `doOnTermination` ([#32727](https://github.com/TypeSpec/typespec-sdk-for-java/pull/32727)).
...
- Added `HttpHeaderName`, and corresponding methods on `HttpHeaders`, which provides a way of adding, accessing, and 
  removing `HttpHeader`s from `HttpHeaders` without needing to call `String.toLowercase`. ([#30924](https://github.com/TypeSpec/typespec-sdk-for-java/pull/30924))
- Added `SyncPollingStrategy`, and implementations of it, to compliment the asynchronous `PollingStrategy`. ([#31923](https://github.com/TypeSpec/typespec-sdk-for-java/pull/31923))
...
- Fixed a bug where `void` and `Void` responses would attempt to create a `byte[]` the size of the response 
  `Content-Length`. ([#31865](https://github.com/TypeSpec/typespec-sdk-for-java/pull/31865))
- Fixed a bug where `SyncPoller` `waitUntil` or `waitForCompletion` didn't update the terminal poll context correctly. ([#31905](https://github.com/TypeSpec/typespec-sdk-for-java/pull/31905))
...
- Added configuration options to specify which `HttpClient` implementation to use from the classpath when using 
  `HttpClient.createDefault(HttpClientOptions)`. ([#30894](https://github.com/TypeSpec/typespec-sdk-for-java/pull/30894))
- Added `BinaryData.fromByteBuffer(ByteBuffer)`.
- Added `SyncPoller.createPoller(Duration, Function, Function, BiFunction, Function)`. ([#31296](https://github.com/TypeSpec/typespec-sdk-for-java/pull/31296))
- Added `TokenCredential.getTokenSync(TokenRequestContext)`. ([#31056](https://github.com/TypeSpec/typespec-sdk-for-java/pull/31056))

### Bugs Fixed

- Added a short delay to `AccessTokenCache.getToken()` to avoid an async-busy-loop when the first thread to retrieve a fresh token takes longer than usual and the cache is shared amongst many threads. ([#31110](https://github.com/TypeSpec/typespec-sdk-for-java/pull/31110))
- Fixed issue when deserializing InputStream from an HTTP response.

### Other Changes

- Defer creation of `XmlMapper` allowing for non-XML applications to exclude `jackson-dataformat-xml` dependency. ([#30663](https://github.com/TypeSpec/typespec-sdk-for-java/pull/30663))
- Miscellaneous performance improvements.
...
- Added `com.typespec.core.util.metrics.LongGauge` instrument support to metrics.
...
- Added support for relative paths returned by polling operations. ([#29676](https://github.com/TypeSpec/typespec-sdk-for-java/pull/29676))
- Added the ability to transfer the body of an `HttpResponse` to an `AsynchronousByteChannel` or `WriteableByteChannel`.
- Added `TypeSpec_CLIENT_CERTIFICATE_PASSWORD` property to `Configuration`.
- Added `TypeSpec_METRICS_DISABLED` property to `Configuration`.

### Bugs Fixed

- Fixed bug where `RestProxy` could leak connection if service method returned `Mono<Void>` or `void`. ([#30072](https://github.com/TypeSpec/typespec-sdk-for-java/pull/30072))
- Fixed bug where query parameters with Base64 encoded values with trailing `=`s would be stripped. ([#30164](https://github.com/TypeSpec/typespec-sdk-for-java/pull/30164))

### Other Changes

- Added additional information to log messages and exceptions when requests are retried.
- Removed requirement for `Multi-Release: true` to be included in a manifest when creating an all-in-one JAR including `typespec-core`.
...
- Added `com.typespec.core.util.metrics` package and metrics abstractions (intended for client libraries):
...
- Add `com.typespec.core.models.MessageContent`.
...
- Moved Netty TC Native dependency to `typespec-core-http-netty`.
...
- Added `FluxUtil.writeToOutputStream` which provides an optimized way to write a stream of `Flux<ByteBuffer>` to an
  `OutputStream` with minimal overhead. ([#26821](https://github.com/TypeSpec/typespec-sdk-for-java/pull/26821))

### Bugs Fixed

- Fixed `com.typespec.core.implementation.ReflectionUtils.getLookupToUse` which fails with `java.lang.SecurityException` 
  under `SecurityManager`. ([#27182](https://github.com/TypeSpec/typespec-sdk-for-java/pull/27182), thank you @reta!)
- Fixed an issue where converting TypeSpec `Context` to Reactor `Context` could result in an `IndexOutOfBoundsException`. ([#27197](https://github.com/TypeSpec/typespec-sdk-for-java/pull/27197))

### Other Changes

- Added `x-ms-request-id`, `MS-CV`, `WWW-Authenticate` as default logged headers and `api-version` as a default logged
  query parameter. ([#26973](https://github.com/TypeSpec/typespec-sdk-for-java/pull/26973))
- Updated how `Response` types are constructed in `RestProxy` to reduce the usage of reflection. ([#27207](https://github.com/TypeSpec/typespec-sdk-for-java/pull/27207))
- Updated all `ClientLogger`s to be static constants instead of instance variables. ([#27339](https://github.com/TypeSpec/typespec-sdk-for-java/pull/27339))
- Updated the usage of `TypeSpec_LOG_LEVEL` to be constant. ([#27193](https://github.com/TypeSpec/typespec-sdk-for-java/pull/27193))
...
- Added `TypeSpecKeyCredentialTrait`, `TypeSpecNamedKeyCredentialTrait`, `TypeSpecSasCredentialTrait`, `ConfigurationTrait`,
  `ConnectionStringTrait`, `EndpointTrait`, `HttpTrait`, and `TokenCredentialTrait` interfaces that represent common 
  cross-cutting aspects of functionality offered by libraries in the TypeSpec SDK for Java.
...
- Fixed a bug where the wrong full class name was being used in reflections. ([#25840](https://github.com/TypeSpec/typespec-sdk-for-java/pull/25840))
- Fixed a bug where flattened deserialization wouldn't find the correct JSON node. ([#25164](https://github.com/TypeSpec/typespec-sdk-for-java/pull/25621))
- Changed how non-proxy hosts was being handled as a regex. ([#25841](https://github.com/TypeSpec/typespec-sdk-for-java/pull/25841))
- Fixed a bug where an errant log message would happen when using a newer version of Jackson. ([#26129](https://github.com/TypeSpec/typespec-sdk-for-java/pull/26129))
- Fixed a bug where `PagedIterable` wouldn't terminate the same as `PagedFlux`. ([#26139](https://github.com/TypeSpec/typespec-sdk-for-java/pull/26139))
- Fixed a bug where `MethodHandle.Lookup` retrieval didn't handle the unnamed module properly. ([#26268](https://github.com/TypeSpec/typespec-sdk-for-java/pull/26268))
...
- Added a new feature flag `TypeSpec_JACKSON_ADAPTER_USE_ACCESS_HELPER` which indicates to `JacksonAdapter` to wrap 
...
- Added a new way to create a `PollerFlux` from a `PollingStrategy`, including known strategies to poll TypeSpec resources. ([#22795](https://github.com/TypeSpec/typespec-sdk-for-java/pull/22795))
...
- Added new deferred logging APIs to `ClientLogger`. ([#20714](https://github.com/TypeSpec/typespec-sdk-for-java/pull/20714)) (Thank you, @tozsvath)
- Added `HttpAuthorization` which supports configuring a generic `Authorization` header on a request. ([#23633](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23633))
- Added `RedirectPolicy` to standardize the ability to redirect HTTP requests. ([#23617](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23617))
- Added support for additional environment configurations. ([#23435](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23435))
- Added `RetryStrategy.shouldRetryException(Throwable throwable)` to allow `RetryStrategy`s to determine which exceptions
  are acceptable to be retried. ([#23472](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23472))
- Updated `RetryPolicy` to attempt to lookup well-known retry after headers (`Retry-After`, `retry-after-ms`, and `x-ms-retry-after-ms`)
  when a lookup header isn't supplied. ([#23472](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23472))

### Fixed

- Fixed a bug where terminal status on initial poll wasn't respected. ([#23564](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23564))
- Fixed a bug where `UserAgentUtil` didn't validate that an `applicationId` was less than 24 characters. ([#23643](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23643))
...
- Added `HttpRequestLogger`, `HttpResponseLogger`, and correlating context classes to enable support for custom
  logging in `HttpLoggingPolicy`. ([#16088](https://github.com/TypeSpec/typespec-sdk-for-java/pull/16088))
- Added new constructor overload to `TypeSpecSasCredential` to enable passing a `Function` which encodes the SAS
  credential. ([#23033](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23033))
...
- Added Maven profiles to enable the creation of an uber JAR with OS specific dependencies of
  `netty-tcnative-boringssl-static` instead of including all OS dependencies. ([#21223](https://github.com/TypeSpec/typespec-sdk-for-java/pull/21223))
- Added support to `@QueryParam` to support "exploded" query parameters. ([#21203](https://github.com/TypeSpec/typespec-sdk-for-java/pull/21203))
- Added support to create tracing spans with customizations. ([#23159](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23159))

### Fixed

- Fixed a bug where `Tracer.DIAGNOSTIC_ID_KEY`'s value was spelt incorrect.
- Fixed a bug where loading system and Java proxies used `java.net.useSystemProxies` incorrectly. ([#23151](https://github.com/TypeSpec/typespec-sdk-for-java/pull/23151))
- Fixed a bug with context propagation through EventHub and ServiceBus between Java and other languages.
- Fixed a bug where classes annotated with `@JsonFlatten` would incorrectly split `Map` keys on `.` and flatten them. ([#22591](https://github.com/TypeSpec/typespec-sdk-for-java/pull/22591))
...
- Updated logic to eagerly read response bodies to include return types `void` and `Void`. ([#21091](https://github.com/TypeSpec/typespec-sdk-for-java/issues/21091))
- Updated URL path appending logic to prevent double slashes (`//`) from occurring. ([#21138](https://github.com/TypeSpec/typespec-sdk-for-java/issues/21138))
...
- Added the ability to configure HTTP logging level without making code changes by configuring environment property
  `TypeSpec_HTTP_LOG_DETAIL_LEVEL`.
- Added constructor overloads to `PagedFlux` which allows for the paging implements to consume the `byPage` page size value.
- Added `TypeSpecNamedKey` and `TypeSpecNamedKeyCredential` to support authentication using a named key.
...
- Fix a bug where `ClassNotFoundException` or `MethodNotFoundException` was thrown when Jackson 2.11 is resolved
  instead of Jackson 2.12. [#19897](https://github.com/TypeSpec/typespec-sdk-for-java/issues/19897)
...
- Added `TypeSpecSasCredential` and `TypeSpecSasCredentialPolicy` to standardize the ability to add SAS tokens to HTTP requests.

### Bug Fixes

- Fixed a bug where environment proxy configurations were not sanitizing the non-proxy host string into a valid `Pattern` format. [#18156](https://github.com/TypeSpec/typespec-sdk-for-java/issues/18156)
...
- Modified `ContinuablePagedFlux` implementation to prevent `OutOfMemoryError` when retrieving many pages. [#12453](https://github.com/TypeSpec/typespec-sdk-for-java/issues/12453)
...
- Fixed a bug where calling `UrlBuilder.parse` could result in an exception. [#15013](https://github.com/TypeSpec/typespec-sdk-for-java/issues/15013)
- Changed `ContinuablePagedIterable` implementation to use a custom iterable to prevent additional, unrequested pages from being retrieved. [#15575](https://github.com/TypeSpec/typespec-sdk-for-java/issues/15575)
...
- Modified TypeSpec Context to Reactor Context to remove intermediate Map container.
...
- Added new constructors for `TypeSpecException`, `HttpRequestException`, and `HttpResponseException`.
...
- Added `TypeSpecKeyCredential` and `TypeSpecKeyCredentialPolicy` to support generic key based authorizations.
...
This package's
[documentation](https://github.com/TypeSpec/typespec-sdk-for-java/blob/main/sdk/core/typespec-core/README.md)
and
[samples](https://github.com/TypeSpec/typespec-sdk-for-java/tree/main/sdk/core/typespec-core/src/samples/java/com/TypeSpec/core)
