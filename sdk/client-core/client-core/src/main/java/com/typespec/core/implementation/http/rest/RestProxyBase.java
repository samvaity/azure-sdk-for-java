...
public abstract class RestProxyBase {
    ...
    public final Object invoke(Object proxy, final Method method, RequestOptions options,
        ...
        try {
            ...
            if (methodParser.isResponseEagerlyRead()) {
                context = context.addData("typespec-eagerly-read-response", true);
            }

            if (methodParser.isResponseBodyIgnored()) {
                context = context.addData("typespec-ignore-response-body", true);
            }

            if (methodParser.isHeadersEagerlyConverted()) {
                context = context.addData("typespec-eagerly-convert-headers", true);
            }
    ...
    public Response createResponse(HttpResponseDecoder.HttpDecodedResponse response, Type entityType,
        ...
        //
        // If the type is either the Response or PagedResponse interface from typespec-core a new instance of either
    ...
    public static HttpResponseException instantiateUnexpectedException(UnexpectedExceptionInformation exception,
        ...
        // For HttpResponseException types that exist in typespec-core, call the constructor directly.
        ...
        if (exceptionType == HttpResponseException.class) {
            ...
            // Finally, if the HttpResponseException subclass doesn't exist in typespec-core, use reflection to create a
            // new instance of it.
            try {
