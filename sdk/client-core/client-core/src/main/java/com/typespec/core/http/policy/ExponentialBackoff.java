...
import static com.typespec.core.util.Configuration.PROPERTY_TypeSpec_REQUEST_RETRY_COUNT;
...
public class ExponentialBackoff implements RetryStrategy {
    ...
    static {
        String envDefaultMaxRetries = Configuration.getGlobalConfiguration().get(PROPERTY_TypeSpec_REQUEST_RETRY_COUNT);
        ...
        if (!CoreUtils.isNullOrEmpty(envDefaultMaxRetries)) {
            try {
                ...
                LOGGER.verbose("{} was loaded but is an invalid number. Using 3 retries as the maximum.",
                    PROPERTY_TypeSpec_REQUEST_RETRY_COUNT);
            }
        }
     ...
     * environment property {@link Configuration#PROPERTY_TypeSpec_REQUEST_RETRY_COUNT}, or three if it isn't configured or
