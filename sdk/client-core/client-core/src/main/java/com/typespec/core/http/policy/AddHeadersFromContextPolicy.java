 ...
 * {@link Context} with key 'typespec-http-headers-key'. The value for this key should be of type {@link HttpHeaders} for
 ...
 *       new Context(AddHeadersFromContextPolicy.TypeSpec_REQUEST_HTTP_HEADERS_KEY, headers));
...
public class AddHeadersFromContextPolicy implements HttpPipelinePolicy {
    ...
    public static final String TypeSpec_REQUEST_HTTP_HEADERS_KEY = "typespec-http-headers-key";
    ...
    private static void addHeadersFromContext(HttpPipelineCallContext context) {
        context.getData(TypeSpec_REQUEST_HTTP_HEADERS_KEY).ifPresent(headers -> {
