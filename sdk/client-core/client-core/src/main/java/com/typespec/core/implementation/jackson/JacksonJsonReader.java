...
// Copied from typespec-core-serializer-json-jackson, with minor edits.
/**
...
final class JacksonJsonReader extends JsonReader {
    ...
    public JsonReader bufferObject() throws IOException {
        ...
        if (currentToken == JsonToken.START_OBJECT || currentToken == JsonToken.FIELD_NAME) {
            ...
            return TypeSpecJsonUtils.createReader(json, jsonOptions);
        } else {
    ...
    public JsonReader reset() throws IOException {
        ...
        return (jsonBytes != null)
            ? TypeSpecJsonUtils.createReader(jsonBytes, jsonOptions)
            : TypeSpecJsonUtils.createReader(jsonString, jsonOptions);
    }

    /*
     * Maps the Jackson Core JsonToken to the typespec-json JsonToken.
     *
     * typespec-json doesn't support the EMBEDDED_OBJECT or NOT_AVAILABLE Jackson Core JsonTokens, but those should only
