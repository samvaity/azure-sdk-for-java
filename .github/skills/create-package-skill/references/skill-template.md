...
The `name` field and directory name MUST match the Maven artifactId (e.g., `azure-search-documents`).
...
### After Regeneration (if TypeSpec-generated)
...
| Rule | Enforced By |
|---|---|
| `name` matches directory name (= Maven artifactId) | `vally lint` |
...
| No cross-language content | `validate_skill_anchors.py` |
